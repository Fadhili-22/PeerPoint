...
const URL = process.argv[2] || "http://127.0.0.1:5599/index.html";
...
async function main() {
  ...
  // Session-scoped websocket helper
  ...
  bws.addEventListener("message", (ev) => {
    ...
    if (m.method === "Runtime.consoleAPICalled") {
      console.log("CONSOLE:", m.params.type, JSON.stringify(m.params.args?.map((a) => a.value)));
    }
  ...
  // Poll until React renders content into #root (max ~20s)
  let rendered = false;
  for (let i = 0; i < 40; i++) {
    await sleep(500);
  ...
  await sleep(1500); // let fonts/paint settle
