...
def list_counsellors(
    ...
    if availability is not None:
        query = query.filter(
            CounsellorProfile.availability_status
            == AvailabilityStatusModel(availability.value)
        )
...
def get_counsellor_availability(
    ...
    """Weekly slots keyed by day_of_week (0–6), plus unavailable dates and
    online status from the profile."""
    profile = _get_active_profile_or_404(db, counsellor_id)
